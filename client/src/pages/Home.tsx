...
function Documents({ documents, references, user, profile, liveMode, onDocumentsChange }: { documents: MasterDocument[]; references: ReferenceRow[]; user: User; profile: Profile | null; liveMode: boolean; onDocumentsChange: (documents: MasterDocument[]) => void }) {
  ...
  const multiPartGroups = useMemo(() => {
    const groups = new Map<string, { order: string; sh: string; parts: ReferenceRow[] }>();
  ...
  return <div className="space-y-7"><SectionHeader eyebrow="Fuente de validación" title="Documentos maestros" description="Carga, filtra y revisa las relaciones del documento maestro. El resumen identifica automáticamente los SH que tienen varios accesorios ligados a una orden." action={<button onClick={() => fileRef.current?.click()} className="primary-action"><UploadCloud className="h-4 w-4" />Cargar archivo</button>} />
