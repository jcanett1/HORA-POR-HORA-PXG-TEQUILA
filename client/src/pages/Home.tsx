...
function ReferencePreview({ references }: { references: ReferenceRow[] }) {
  if (references.length === 0) return <tbody className="divide-y divide-slate-100"><tr><td colSpan={5} className="px-6 py-8 text-center text-xs text-slate-500">Aún no hay filas activas en datos_referencia.</td></tr></tbody>;
  return <tbody className="divide-y divide-slate-100">{references.slice(0, 4).map((item, index) => <tr key={item.id}><td className="px-6 py-4 text-xs text-slate-500">{item.numero_fila_origen || index + 1}</td><td className="px-5 py-4 font-mono text-xs font-bold text-slate-700">{item.orden_original}</td><td className="px-5 py-4 font-mono text-xs text-slate-600">{item.numero_parte_original}</td><td className="px-5 py-4 font-mono text-xs text-slate-600">{item.sh_original}</td><td className="px-6 py-4"><StatusPill state="Confirmado" /></td></tr>)}</tbody>;
}
...
export default function Home({ user, profile, liveMode, signOut }: AuthContextValue) {
  ...
  useEffect(() => {
    ...
    void fetchTraceData(user, profile)
      ...
      .catch((error) => toast.error(error instanceof Error ? `No fue posible cargar los datos de Supabase: ${error.message}` : "No fue posible cargar los datos de Supabase."))
