...
function Capture({ onCapture, user, profile, liveMode, operatorName, onProfileChange }: { onCapture: (record: RecordItem) => void; user: User; profile: Profile | null; liveMode: boolean; operatorName: string; onProfileChange: (profile: Profile) => void }) {
  ...
  async function submitCapture(event: React.FormEvent) {
    ...
    if (!quantity && accessoryRows.length === 0) { toast.error("Completa la cantidad del material."); return; }
    ...
    const items = accessoryRows.length > 0
    ...
    <div className="grid gap-5 xl:grid-cols-[1.4fr_.8fr]">
      <form onSubmit={submitCapture} className="soft-card overflow-hidden">
        ...
        <div className="space-y-5 p-5 sm:p-7">
          ...
          {accessoryRows.length > 0 ? <div className="rounded-2xl border border-cyan-100 bg-cyan-50/60 p-4"><div className="flex items-center justify-between gap-3"><div><p className="text-sm font-bold text-cyan-950">Accesorios que debes capturar</p><p className="mt-1 text-xs text-cyan-800">El SH fue encontrado en los documentos maestros. Confirma o corrige el código y captura la cantidad de cada uno. Se guardará un match independiente por accesorio.</p></div><span className="rounded-full bg-white px-2.5 py-1 text-[11px] font-bold text-cyan-800">{accessoryRows.length} encontrados</span></div><div className="mt-4 grid gap-3 md:grid-cols-2">{accessoryRows.map((row, index) => <div key={`${row.reference.id}-${row.reference.order}`} className="rounded-xl border border-cyan-100 bg-white p-3"><div className="mb-2 flex items-center justify-between gap-2"><span className="text-[10px] font-bold uppercase tracking-[.1em] text-slate-400">Accesorio {index + 1} · SO {row.reference.order}</span>{row.reference.expectedQuantity !== null ? <span className="text-[10px] font-semibold text-slate-400">Esperado: {row.reference.expectedQuantity}</span> : null}</div><div className="grid gap-2 sm:grid-cols-2"><input aria-label={`Código accesorio ${index + 1}`} value={row.code} onChange={(e) => updateAccessory(index, "code", e.target.value)} className="field-input font-mono text-xs" placeholder="Código" /><input aria-label={`Cantidad accesorio ${index + 1}`} type="number" min="0" step="0.001" value={row.quantity} onChange={(e) => updateAccessory(index, "quantity", e.target.value)} className="field-input font-mono text-xs" placeholder="Cantidad" /></div></div>)}</div></div> : null}
