...
function RelationTable({ rows, title, description }: { rows: RelationRow[]; title: string; description: string }) {
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((row) => [row.order, row.code, row.description, row.shipment].some((value) => value.toLowerCase().includes(normalized)));
  }, [query, rows]);
  return (
    <div className="soft-card overflow-hidden">
      <div className="flex flex-col gap-3 border-b border-slate-100 px-5 py-5 sm:flex-row sm:items-end sm:justify-between sm:px-6">
        <div><p className="text-sm font-bold text-slate-800">{title}</p><p className="mt-1 text-xs text-slate-500">{description} · {formatNumber(rows.length)} filas</p></div>
...
function AppearanceTable({ rows, category }: { rows: AppearanceRow[]; category: ProductCategory }) {
  ...
  const filtered = useMemo(() => {
    const normalized = query.trim().toLowerCase();
    return normalized ? rows.filter((row) => `${row.code} ${row.description}`.toLowerCase().includes(normalized)) : rows;
  }, [query, rows]);
  return (
    <div className="soft-card overflow-hidden">
      <div className="flex flex-col gap-3 border-b border-slate-100 px-5 py-5 sm:flex-row sm:items-end sm:justify-between sm:px-6"><div><p className="text-sm font-bold text-slate-800">Resumen de apariciones · {category}</p><p className="mt-1 text-xs text-slate-500">Código, descripción y número de apariciones.</p></div><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar código o descripción" className="h-9 w-full rounded-lg border border-slate-200 bg-white px-3 text-xs outline-none transition focus:border-cyan-500 focus:ring-3 focus:ring-cyan-100 sm:w-64" /></div>
      {filtered.length ? <div className="max-h-[390px] overflow-auto"><table className="w-full min-w-[620px] text-left"><thead className="sticky top-0 z-10 bg-slate-50/95 text-[10px] font-bold uppercase tracking-[.12em] text-slate-500 backdrop-blur"><tr><th className="px-5 py-3">Código</th><th className="px-5 py-3">Descripción</th><th className="px-5 py-3">Apariciones</th></tr></thead><tbody className="divide-y divide-slate-100">{filtered.map((row) => <tr key={row.code} className="hover:bg-cyan-50/35"><td className="whitespace-nowrap px-5 py-3 font-mono text-[11px] text-cyan-800">{row.code}</td><td className="px-5 py-3 text-xs text-slate-600">{row.description}</td><td className="px-5 py-3 text-sm font-bold text-slate-800">{formatNumber(row.appearances)}</td></tr>)}</tbody></table></div> : <div className="p-5"><EmptyState message="No se encontraron apariciones para esta categoría." /></div>}
    </div>
  );
}
