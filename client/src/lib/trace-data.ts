...
export function mapCaptureRow(row: CaptureRow): TraceRecord {
  ...
  return {
    ...
    part: row.numero_parte_original || "Sin accesorio",
    sh: row.sh_original,
    ...
    reason: !row.numero_parte_original.trim() ? "Registro general por hora sin accesorio; no se realizó match." : row.motivo_discrepancia || undefined,
